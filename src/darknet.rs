...
impl Block {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => candle_core::bail!("cannot find {} in {}", key, self.block_type),
            Some(value) => Ok(value),
        }
    }
}
...
impl Darknet {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => candle_core::bail!("cannot find {} in net parameters", key),
            Some(value) => Ok(value),
        }
    }
}
...
pub fn parse_config<T: AsRef<Path>>(path: T) -> Result<Darknet> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        if line.starts_with('[') {
            if !line.ends_with(']') {
                candle_core::bail!("line does not end with ']' {line}")
            }
            ...
            if key_value.len() != 2 {
                candle_core::bail!("missing equal {line}")
            }
            ...
            if prev.is_some() {
                candle_core::bail!("multiple value for key {}", line)
            }
        }
    }
    acc.finish_block();
...
fn conv(vb: VarBuilder, index: usize, p: usize, b: &Block) -> Result<(usize, Bl)> {
    ...
    let leaky = match activation {
        "leaky" => true,
        "linear" => false,
        otherwise => candle_core::bail!("unsupported activation {}", otherwise),
    };
...
fn yolo(p: usize, block: &Block) -> Result<(usize, Bl)> {
    ...
    if flat.len() % 2 != 0 {
        candle_core::bail!("even number of anchors");
    }
    ...
    pub fn build_model(&self, vb: VarBuilder) -> Result<Func> {
        ...
        for (index, block) in self.blocks.iter().enumerate() {
            let channels_and_bl = match block.block_type.as_str() {
                ...
                otherwise => candle_core::bail!("unsupported block type {}", otherwise),
            };
